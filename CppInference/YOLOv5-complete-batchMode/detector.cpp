...
std::vector<std::vector<Detection>> Detector::Forward(std::vector<cv::Mat> &images,int num_classes)
{
    std::vector<int64_t> inputDim = {images.size(),3, 640, 640};
    if(num_classes>0){
        outputDims[0][0] = images.size();
        outputDims[0][1] = 3*21*((inputDim[2]*inputDim[3])/(32*32));
        outputDims[0][2] = 5 + num_classes;
    }
    ...
    for(int b=0;b<images.size();b++){
        cv::Mat img = images[b];
        int INPUT_H = img.size().height;
        int INPUT_W = img.size().width;
        cv::Mat pr_img;

        auto pad_info = LetterboxImage(img,pr_img, cv::Size(inputDim.at(2), inputDim.at(3)) );
        pads.push_back(pad_info);
...
//        }
        ...
        cv::dnn::blobFromImage(pr_img, pr_img);
        pr_img.convertTo(pr_img, CV_32F, 1.0 / 255.0);


        memcpy(&input_data[b*vectorProduct(inputDim)/images.size()], pr_img.data,vectorProduct(inputDim)/images.size()*sizeof(float));
    }
    ...
    for(int b=0;b<images.size();b++){
        auto result = postProcessCpu(&outputTensors[0].GetTensorData<float>()[b*vectorProduct(outputDims[0])/images.size()],outputDims[0],threshold,0.6, pads[b]);
